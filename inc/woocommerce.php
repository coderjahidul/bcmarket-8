    ...
    <table class="list zebra ac">
        ...
        foreach ( $results as $item ) : ?>
            <?php 
                ...
                if ( $order ) {
                    $download_expired = ( time() - $order->get_date_created()->getTimestamp() ) > ( 180 * DAY_IN_SECONDS );
                }
                // Skip rendering sold items once the 90-day window has passed.
                if ( $item->item_status === 'sold' && $download_expired ) {
                    continue;
                }
            ?>
...
/**
 * When line items are missing but sold rows exist in the accounts table, rebuild products on the order.
 * Runs once per order (meta _bcmarket_line_items_repaired) when an admin opens the order screen.
 */
function bcmarket_repair_missing_order_line_items_from_accounts( $order ) {
	...
	if ( (float) $order->get_total() <= 0.00001 ) {
		return;
	}
	if ( $order->get_meta( '_bcmarket_line_items_repaired' ) ) {
		return;
	}
	...
	$groups = $wpdb->get_results(
		$wpdb->prepare(
			"SELECT product_id, COUNT(*) AS qty FROM {$table} WHERE order_id = %d AND product_id > 0 GROUP BY product_id",
			$order->get_id()
		),
		ARRAY_A
	);
	...
	$order->update_meta_data( '_bcmarket_line_items_repaired', '1' );
	$order->save();
}

add_action( 'woocommerce_admin_order_data_after_billing_address', 'bcmarket_repair_missing_order_line_items_from_accounts', 1 );
